...
#define MOUSE_PHASE_ERROR 0
#define MOUSE_PHASE_UNINIT 1
#define MOUSE_PHASE_ONE 2
#define MOUSE_PHASE_TWO 3
#define MOUSE_PHASE_THREE 4
...
void erase_mouse(unsigned char *vram) {
    boxfill8(vram, g_xsize, COL8_008484, g_mx, g_my, g_mx + 15, g_my + 15);
}

void draw_mouse(unsigned char *vram) {
    put_block(vram, g_xsize, 16, 16, g_mx, g_my, g_mcursor, 16);
}
...
int mouse_decode(unsigned char dat) {
    ...
    if (g_mdec.m_phase == MOUSE_PHASE_THREE) {
        ...
        g_mdec.m_x = g_mdec.m_buf[1];
        g_mdec.m_y = g_mdec.m_buf[2];

        if ((g_mdec.m_buf[0] & 0x10) != 0) {
            g_mdec.m_x |= 0xffffff00;
        }

        if ((g_mdec.m_buf[0] & 0x20) != 0) {
            g_mdec.m_y |= 0xffffff00;
        }

        g_mdec.m_y = -g_mdec.m_y;
        return 1;
    }
